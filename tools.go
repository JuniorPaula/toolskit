...
// ReadJSON tries to read the body of request and converts from json into a go data variable
...
// WriteJSON takes a response status code and arbitrary data and wrires json to the client
...
// ErrorJSON takes an error, and optionally a status code, end generates and send a JSON error message
...
// PushJSONToRemote post arbitrary data some URL as JSON, and returns the response, status code, and error, if any.
// The final parameter, client, is optional. If none is specifield, we used the stardard http.Client.
